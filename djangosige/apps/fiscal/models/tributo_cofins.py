...
class TributoCOFINS(models.Model):
    ...
    class Meta:
        verbose_name = "Tributo COFINS"
        verbose_name_plural = "Tributos COFINS"
    ...
    def save(self, *args, **kwargs):
        """Calcula valor_cofins automaticamente"""
        if self.aliquota and self.valor_bc:
            self.valor_cofins = self.valor_bc * (self.aliquota / 100)
    ...
    def to_edoc(self):
        ...
        cofins = {
            'CST': self.cst,
            'vCOFINS': str(self.valor_cofins)
        }
        
        if self.valor_bc:
            cofins['vBC'] = str(self.valor_bc)
        
        if self.aliquota:
            cofins['pCOFINS'] = str(self.aliquota)
        
        return {
            'COFINS': {
                'COFINS'+self.cst: cofins
            }
        }